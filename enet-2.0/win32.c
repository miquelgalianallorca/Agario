...
enet_address_set_sin (struct sockaddr * sin, const ENetAddress * address, ENetAddressFamily family)
{
    ...
    else if (family == ENET_IPV6)
    {
        ...
        addr6 -> sin6_addr     = *(struct in6_addr *) & address->host.addr;
        addr6 -> sin6_scope_id = address -> scopeID;
        addr6 -> sin6_port     = ENET_HOST_TO_NET_16 (address -> port);
        return 0;
    }
    return -1;
}
...
enet_socket_bind (ENetSocket socket, const ENetAddress * address, ENetAddressFamily family)
{
    ...
    struct sockaddr_in6* saddr = (struct sockaddr_in6 *) & sin;
    int iRet = bind(socket, (struct sockaddr *) & sin, enet_sa_size(family));

    return  iRet == SOCKET_ERROR ? -1 : 0;
}
